const introTemplates = [
  (area: any, category: any) =>
    ...
    `ISI-certified ${category.display_name} starting at ₹${category.price_per_sheet}. ` +
...
export function generateFAQ(area: any, category: any): any[] {
  return [
    ...
    },
    {
      ...
      answer: `${category.display_name} price in ${area.name} starts at ₹${category.price_per_sheet} per sheet. Being ${area.distance_km}km from Karur, transport cost is minimal. Bulk orders for ${area.famous_for} businesses get special rates.`
    },
    {

    ...
    <div className="carp-proj-card">
      <div className="carp-proj-img" onClick={() => allPhotos.length > 0 && open(0)} style={{ cursor: allPhotos.length > 0 ? 'pointer' : 'default' }}>
        {p.cover_image
          ...
          : <div style={{ height:'100%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:48, background:'linear-gradient(135deg,#0d1f3a,#19376D)' }}>🔨</div>
        }
        {p.year && (
          <div style={{ position:'absolute', top:10, right:10, background:'rgba(7,15,31,0.85)', backdropFilter:'blur(6px)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:3, padding:'3px 10px', fontSize:10, fontFamily:"'Syne',sans-serif", fontWeight:700, color:'#7A8EA8' }}>{p.year}</div>
        )}
        {allPhotos.length > 1 && (
          <div style={{ position:'absolute', bottom:10, right:10, background:'rgba(7,15,31,0.85)', backdropFilter:'blur(6px)', border:'1px solid rgba(249,115,22,0.25)', borderRadius:3, padding:'3px 10px', fontSize:10, fontFamily:"'Syne',sans-serif", fontWeight:700, color:'#F97316' }}>
            📷 {allPhotos.length} photos
          </div>
        )}
        <div className="carp-proj-overlay"><span style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:12, color:'white' }}>View Gallery ↗</span></div>
      </div>
      {allPhotos.length > 1 && (
      ...
      <div style={{ padding:'16px 18px 20px' }}>
        <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:'0.95rem', color:'#F8F9FB', marginBottom:5 }}>{p.title}</div>
        {p.location && <div style={{ fontSize:12, color:'#7A8EA8', marginBottom:8 }}>📍 {p.location}</div>}
        {p.description && <p style={{ fontSize:13, color:'#7A8EA8', lineHeight:1.65, marginBottom:12 }}>{p.description}</p>}
        {p.materials_used?.length > 0 && (
          <div style={{ display:'flex', flexWrap:'wrap', gap:5 }}>
            {p.materials_used.map(m => (
              <span key={m} style={{ fontSize:10, fontFamily:"'Syne',sans-serif", fontWeight:700, background:'rgba(249,115,22,0.08)', border:'1px solid rgba(249,115,22,0.2)', color:'#F97316', padding:'2px 8px', borderRadius:3, letterSpacing:'.06em' }}>{m}</span>
            ))}
          </div>
        )}
      </div>
...
function Stars({ rating, size = 14 }: { rating: number; size?: number }) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:3 }}>
      {[1,2,3,4,5].map(n => (
        <span key={n} style={{ color: n <= Math.round(rating) ? '#F97316' : 'rgba(249,115,22,0.2)', fontSize:size }}>★</span>
      ))}
    ...
    <div style={{ display:'flex', gap:4 }}>
      {[1,2,3,4,5].map(n => (
        <button
          ...
          style={{ background:'none', border:'none', cursor:'pointer', fontSize:28, color: n <= (hover || value) ? '#F97316' : 'rgba(249,115,22,0.2)', transition:'color 0.1s', padding:'0 2px', lineHeight:1 }}
...
function ReviewForm({ carpenterId, carpenterName }: { carpenterId: string; carpenterName: string }) {
  ...
  const inp: React.CSSProperties = {
    ...
    border: '1px solid rgba(249,115,22,0.18)', borderRadius: 6,
    padding: '10px 14px', fontSize: 14, color: '#F8F9FB',
  ...
  if (done) return (
    <div style={{ textAlign:'center', padding:'32px 0' }}>
      ...
      <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:'1.8rem', letterSpacing:'0.05em', color:'#F8F9FB', marginBottom:8 }}>THANK YOU!</div>
      <p style={{ color:'#7A8EA8', fontSize:14, lineHeight:1.7 }}>
  ...
  return (
    <div>
      <div style={{ marginBottom:18 }}>
        <label style={{ display:'block', fontSize:11, fontFamily:"'Syne',sans-serif", fontWeight:700, letterSpacing:'0.15em', textTransform:'uppercase', color:'#7A8EA8', marginBottom:8 }}>
          Your Rating *
        </label>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:14 }} className="review-form-grid">
        <div>
          <label style={{ display:'block', fontSize:11, fontFamily:"'Syne',sans-serif", fontWeight:700, letterSpacing:'0.15em', textTransform:'uppercase', color:'#7A8EA8', marginBottom:6 }}>Your Name *</label>
          <input style={inp} placeholder="Rajan Kumar" value={form.name} onChange={e => set('name', e.target.value)} />
        </div>
        <div>
          <label style={{ display:'block', fontSize:11, fontFamily:"'Syne',sans-serif", fontWeight:700, letterSpacing:'0.15em', textTransform:'uppercase', color:'#7A8EA8', marginBottom:6 }}>Your Role / City</label>
          <input style={inp} placeholder="e.g. Homeowner, Karur" value={form.role} onChange={e => set('role', e.target.value)} />
        </div>
      </div>

      <div style={{ marginBottom:18 }}>
        <label style={{ display:'block', fontSize:11, fontFamily:"'Syne',sans-serif", fontWeight:700, letterSpacing:'0.15em', textTransform:'uppercase', color:'#7A8EA8', marginBottom:6 }}>Your Review *</label>
        <textarea
      ...
      <button
        ...
        disabled={loading}
        style={{ width:'100%', padding:'12px 0', borderRadius:6, background: loading ? 'rgba(249,115,22,0.4)' : '#F97316', color:'#0B2447', border:'none', fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:13, letterSpacing:'0.1em', textTransform:'uppercase', cursor: loading ? 'default' : 'pointer', transition:'background 0.2s' }}
      >
        {loading ? '⏳ Submitting...' : '⭐ Submit Review'}
      </button>

      <p style={{ fontSize:12, color:'#7A8EA8', textAlign:'center', marginTop:10 }}>
        Reviews appear after a quick verification by our team.
      </p>
      <style>{`input:focus,textarea:focus{border-color:#F97316!important}`}</style>
    </div>
  );
}
...
function ReviewCard({ r }: { r: Review }) {
  return (
    <div style={{ background:'rgba(25,55,109,0.3)', border:'1px solid rgba(249,115,22,0.1)', borderRadius:10, padding:'18px 20px' }}>
      <div style={{ display:'flex', alignItems:'center', gap:4, marginBottom:8 }}>
        {[1,2,3,4,5].map(n => (
          <span key={n} style={{ color: n <= r.rating ? '#F97316' : 'rgba(249,115,22,0.2)', fontSize:14 }}>★</span>
        ))}
      </div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10 }}>
        <div style={{ width:32, height:32, borderRadius:'50%', background:'linear-gradient(135deg,#F97316,#FF9A45)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:13, color:'#0B2447', flexShrink:0 }}>
          {r.name.charAt(0).toUpperCase()}
        </div>
        <div>
          <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:13, color:'#F8F9FB' }}>{r.name}</div>
          {r.role && <div style={{ fontSize:11, color:'#7A8EA8' }}>{r.role}</div>}
        </div>
      </div>
    </div>
  );
...
export default function CarpenterPortfolioPage() {
  ...
  if (loading) return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#070F1F', color:'#7A8EA8' }}>
      ⏳ Loading portfolio...
    </div>
  );

  if (!carp) return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#070F1F', flexDirection:'column', gap:16 }}>
      <div style={{ fontSize:48 }}>🔨</div>
      <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:'2rem', color:'#F8F9FB' }}>NOT FOUND</div>
      <Link href="/carpenters" style={{ fontSize:13, color:'#F97316', fontFamily:"'Syne',sans-serif", fontWeight:700 }}>← Back to Directory</Link>
    </div>
  );
  ...
  return (
    <>
      {/* Hero */}
      <section style={{ background:'linear-gradient(160deg,#0a1d3a,#070F1F)', borderBottom:'1px solid rgba(249,115,22,0.15)', padding:'80px 0 56px', paddingTop:'calc(58px + 80px)' }}>
        <div style={{ maxWidth:1100, margin:'0 auto', padding:'0 5rem' }} className="carp-port-pad">
          ...
          <div style={{ display:'flex', gap:24, alignItems:'flex-start', flexWrap:'wrap' }}>
            <div style={{ width:88, height:88, borderRadius:'50%', background:'rgba(249,115,22,0.1)', border:'3px solid rgba(249,115,22,0.3)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:38, overflow:'hidden', position:'relative', flexShrink:0 }}>
            ...
            <div style={{ flex:1 }}>
              <div style={{ display:'flex', gap:8, marginBottom:10, flexWrap:'wrap' }}>
                {carp.verified && (
                  <span style={{ fontSize:10, fontFamily:"'Syne',sans-serif", fontWeight:700, letterSpacing:'.12em', textTransform:'uppercase', background:'rgba(37,211,102,0.12)', color:'#4ADE80', border:'1px solid rgba(37,211,102,0.2)', padding:'3px 10px', borderRadius:2 }}>✓ Verified</span>
                )}
              </div>
              <h1 style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:'clamp(2rem,4vw,3rem)', letterSpacing:'.04em', color:'#F8F9FB', lineHeight:1, marginBottom:8 }}>{carp.name}</h1>
              <div style={{ fontSize:13, color:'#7A8EA8', marginBottom:10 }}>📍 {carp.area} · {carp.experience}+ years experience</div>
              {avgRating > 0 && (
                <div style={{ marginBottom:12, display:'flex', alignItems:'center', gap:10 }}>
                  <Stars rating={avgRating} />
                  {reviews.length > 0 && <span style={{ fontSize:12, color:'#7A8EA8' }}>({reviews.length} review{reviews.length !== 1 ? 's' : ''})</span>}
                </div>
              )}
              ...
              {carp.speciality?.length > 0 && (
                <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:22 }}>
                  {carp.speciality.map((s: string) => (
                    <span key={s} style={{ fontSize:11, fontFamily:"'Syne',sans-serif", fontWeight:700, letterSpacing:'.08em', textTransform:'uppercase', background:'rgba(249,115,22,0.08)', border:'1px solid rgba(249,115,22,0.2)', color:'#F97316', padding:'3px 9px', borderRadius:2 }}>{s}</span>
                  ))}
                </div>
              )}
            ...
            {projects.length > 0 && (
              <div style={{ background:'rgba(25,55,109,0.4)', border:'1px solid rgba(249,115,22,0.15)', borderRadius:8, padding:'20px 28px', textAlign:'center', flexShrink:0 }}>
                <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:'2.8rem', letterSpacing:'.04em', color:'#F97316', lineHeight:1 }}>{projects.length}</div>
                <div style={{ fontSize:10, fontFamily:"'Syne',sans-serif", fontWeight:700, letterSpacing:'.15em', textTransform:'uppercase', color:'#7A8EA8', marginTop:4 }}>Projects</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ maxWidth:1100, margin:'0 auto', padding:'0 5rem' }} className="carp-port-pad">
          {projects.length > 0 && (
            <>
              ...
              <h2 style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:'clamp(1.8rem,3vw,2.8rem)', letterSpacing:'.04em', color:'#F8F9FB', marginBottom:32 }}>COMPLETED WORK</h2>
          ...
          <div style={{ marginTop:projects.length > 0 ? 64 : 0 }}>
            ...
            <h2 style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:'clamp(1.8rem,3vw,2.8rem)', letterSpacing:'.04em', color:'#F8F9FB', marginBottom:32 }}>
            ...
            <div className="reviews-layout">
              ...
              <div>
                {reviews.length === 0 ? (
                  <div style={{ padding:'32px 24px', background:'rgba(25,55,109,0.25)', border:'1px solid rgba(249,115,22,0.1)', borderRadius:10, textAlign:'center', marginBottom:0 }}>
                    <div style={{ fontSize:36, marginBottom:10 }}>⭐</div>
                    <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:14, color:'#F8F9FB', marginBottom:6 }}>No reviews yet</div>
                    <p style={{ fontSize:13, color:'#7A8EA8' }}>Be the first to share your experience!</p>
                  </div>
                ) : (
              ...
              <div style={{ background:'rgba(25,55,109,0.3)', border:'1px solid rgba(249,115,22,0.15)', borderRadius:12, padding:'28px 24px' }}>
                <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:'1.5rem', letterSpacing:'.05em', color:'#F8F9FB', marginBottom:6 }}>WRITE A REVIEW</div>
                <p style={{ fontSize:13, color:'#7A8EA8', marginBottom:22, lineHeight:1.6 }}>
          ...
          {/* CTA */}
          <div style={{ marginTop:52, background:'linear-gradient(135deg,#0d1f3a,#19376D)', border:'1px solid rgba(249,115,22,0.2)', borderRadius:10, padding:'32px 44px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:24, flexWrap:'wrap' }}>
            <div>
              <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:'1.6rem', letterSpacing:'.05em', color:'#F8F9FB', marginBottom:6 }}>NEED A SKILLED CARPENTER?</div>
              <p style={{ fontSize:13, color:'#7A8EA8' }}>{carp.name} uses quality materials from Karur Plywood &amp; Company.</p>
            </div>
          ...
          <div style={{ marginTop:28, textAlign:'center' }}>
            <Link href="/carpenters" style={{ fontSize:13, color:'#7A8EA8', fontFamily:"'Syne',sans-serif", fontWeight:700, letterSpacing:'.08em' }}>
      ...
      <style>{`
        ...
        .carp-proj-card { background:rgba(25,55,109,0.35); border:1px solid rgba(249,115,22,0.15); border-radius:10px; overflow:hidden; transition:border-color .25s,transform .25s,box-shadow .25s; }
        ...
        .carp-proj-overlay { position:absolute; inset:0; background:rgba(7,15,31,0.55); display:flex; align-items:center; justify-content:center; opacity:0; transition:opacity .25s; }
        ...
        .lb-prev, .lb-next { position:absolute; top:50%; transform:translateY(-50%); background:rgba(7,15,31,0.8); border:1px solid rgba(249,115,22,0.25); border-radius:4px; color:#F97316; width:44px; height:44px; cursor:pointer; font-size:26px; display:flex; align-items:center; justify-content:center; z-index:2; }

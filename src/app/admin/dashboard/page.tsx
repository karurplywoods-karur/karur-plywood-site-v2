...
// Full file with BOM tab integrated. Replace your existing dashboard/page.tsx with this.
...
type Tab = 'products' | 'bulk' | 'enquiries' | 'reviews' | 'bom';
...
export default function AdminDashboard() {
  ...
  const openEdit = (p: Product) => {
    setForm({ name:p.name, category_id:p.category_id||'', description:p.description, image_url:p.image_url, type:p.type, price:p.price||'', unit:p.unit, in_stock:p.in_stock });
  ...
  const tabBtn = (t: Tab, label: string) => (
    <button key={t} onClick={()=>setTab(t)} style={{ padding:'9px 20px',borderRadius:8,border:'none',cursor:'pointer',fontFamily:'Outfit,sans-serif',fontWeight:600,fontSize:13,
  ...
  return (
    <div style={{ minHeight:'100vh',background:'#0E0B08',color:'#F0E8DC',fontFamily:'Outfit,sans-serif' }}>
      ...
      <div style={{ maxWidth:1200,margin:'0 auto',padding:'28px 28px' }}>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14,marginBottom:28 }} className="stats-grid">
          {[
            ...
            {icon:'📄',num:0,label:'BOM Requests',sub:'Material list quotes'},
          ].map(s=>(
	  ...
	  <a href="/admin/orders" style={{ padding:'9px 20px', borderRadius:8, fontFamily:'Outfit,sans-serif', fontWeight:600, fontSize:13, background:'transparent', color:'#9A8070', textDecoration:'none', display:'flex', alignItems:'center' }}>🛒 Orders ↗</a>
